...
class Createur extends Model
{
    ...
    public function connecter($email, $motDePasse)
    {
        ...
        if ($createur && password_verify($motDePasse, $createur['mdp_createur'])) {
    ...
    public function emailExistant(string $email): bool
    {
        ...
        return $this->findOneBy(['ad_mail_createur' => $email]) !== null;
    }
    public function getByEmail(string $email)
{
    // Ici, tu devras effectuer une requête pour récupérer l'utilisateur par e-mail.
    $query = "SELECT * FROM createur WHERE ad_mail_createur = :email";
